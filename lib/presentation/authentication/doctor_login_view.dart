  ...
  Future<void> handleSignIn() async {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      try {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => DoctorMainView()),
        );
      } catch (e) {
    ...
    return BlocConsumer<AuthCubit, AuthState>(
      ...
      listener: (context, state) {
        if (state.status == AuthStatus.authenticated) {
          getIt<AppRouter>().pushAndRemoveUntil(const DoctorMainView());
                    ...
                    CustomButton(
                      onPressed: () async {
                        if (_formKey.currentState!.validate()) {
                          ...
                          // Navigasyon artık listener'da, burada yapmıyoruz
                        }
                      },
                      text: 'Login',
                      child:
